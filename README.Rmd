...
Nous pouvons aussi compter combien de contrats **commencent** chaque année :
...
Nous pouvons enfin quantifier, chaque année, combien de projets sont **en cours**.
...
Nous pouvons représenter ces deux variables (nombre de publications / nombre de relectures) sur un même graphique :

```{r out.width = "100%", fig.width = 17, fig.height = 15}
...
Nous pouvons donc quantifier le nombre d'auteurs ETBX pour chaque publication **(Seul le top 5 est présenté dans ce document)**.
...
Avec l'information du nombre de co-auteurs, nous pouvons corriger le nuage de mot précédent en le pondérant par le nombre moyen de co-auteurs ETBX par publication pour chaque revue.
...
Nous pouvons maintenant nous intéresser aux disciplines **(Seul le top 5 est présenté dans ce document)**.
...
Nous pouvons également observer les publications avec plus d'1 auteur ETBX mais mono-disciplinaires :
...
Un premier travail de catégorisation manuelle de nos partenaires a été entamé par le GT (B. Rulleau), mais ne donne pour le moment pas suffisamment d'informations pour pousser l'analyse avec ces données.

#### Nombre de citations (A faire)

A partir de l'export HAL-INRAE, j'ai pu récupérer `r n_distinct(HAL$doiId_s)` DOI, que je pourrais utiliser pour récupérer des données du nombre de citations de chaque article via Scopus. 
...
- [] Recensement des partenaires économiques/externes : Donnée inexistante sur le fichier excel, par quel moyen y accéder ?
